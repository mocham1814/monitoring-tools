  ...
  export default {
    ...
    methods: {
      ...
      load_data: function(id_jabatan) {
        ...
        self.loading = true;
        axios
          .get(`${this.$axios.defaults.baseURL}/master/users/get_users`,
              {
              headers: {
                Authorization:  auth
              }
          })
          .then(response => (this.users = response.data), setTimeout(() => (self.loading = false), 2))
      ...
      addItem () {
          this.$router.push('/users/user_add');
      },

      editItem (item) {
        this.$router.push({path: '/users/user_edit', query: {id:item}})
      },
      ...
      deleteItem (item) {
        ...
        let self = this;
        axios
        .get(`${this.$axios.defaults.baseURL}/master/users/delete_user/` + item,{
            headers: {
              Authorization:  auth
            }
        })
